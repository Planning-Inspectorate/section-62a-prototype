...
router.get('/current-service/back-office/tab-view/09-outcome', function(req, res) {
  ...
  // Render the page
  res.render('current-service/back-office/tab-view/09-outcome', { 
...
router.post('/publish-status-answer-09-outcome', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/09-outcome', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/09-outcome?reference=${foundCase.reference}`);
});
