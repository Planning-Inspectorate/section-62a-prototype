...
router.get('/current-service/back-office/tab-view/15-vehicle-parking', function(req, res) {
  ...
  // Render the page
  res.render('current-service/back-office/tab-view/15-vehicle-parking', { 
...
router.post('/publish-status-answer-15-vehicle-parking', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/15-vehicle-parking', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/15-vehicle-parking?reference=${foundCase.reference}`);
});
