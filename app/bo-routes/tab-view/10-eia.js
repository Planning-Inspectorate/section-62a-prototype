...
router.get('/current-service/back-office/tab-view/10-eia', function(req, res) {
  ...
  // Render the page
  res.render('current-service/back-office/tab-view/10-eia', { 
...
router.post('/publish-status-answer-10-eia', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/10-eia', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/10-eia?reference=${foundCase.reference}`);
});
