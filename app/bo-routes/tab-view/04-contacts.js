...
router.get('/current-service/back-office/tab-view/04-contacts', function(req, res) {
  ...
  // Render the page
  res.render('current-service/back-office/tab-view/04-contacts', { 
...
router.post('/publish-status-answer-04-contacts', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/04-contacts', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/04-contacts?reference=${foundCase.reference}`);
});
