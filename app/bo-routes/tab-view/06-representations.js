...
router.post('/publish-status-answer-06-representations', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/06-representations', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/06-representations?reference=${foundCase.reference}`);
});
