...
router.get('/current-service/back-office/tab-view/17-pre-application', function(req, res) {
  ...
  // Render the page
  res.render('current-service/back-office/tab-view/17-pre-application', { 
...
router.post('/publish-status-answer-17-pre-application', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/17-pre-application', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/17-pre-application?reference=${foundCase.reference}`);
});
