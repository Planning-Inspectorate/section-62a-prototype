...
router.get('/current-service/back-office/tab-view/08-event', function(req, res) {
  ...
  // Render the page
  res.render('current-service/back-office/tab-view/08-event', { 
...
router.post('/publish-status-answer-08-event', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/08-event', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/08-event?reference=${foundCase.reference}`);
});
