...
router.get('/current-service/back-office/tab-view/01-overview', function(req, res) {
  ...
  // Render the page
  res.render('current-service/back-office/tab-view/01-overview', { 
...
router.post('/publish-status-answer-01-overview', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/01-overview', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/01-overview?reference=${foundCase.reference}`);
});
