...
router.get('/current-service/back-office/tab-view/02-details', function(req, res) {
  ...
  // Render the page
  res.render('current-service/back-office/tab-view/02-details', { 
...
router.post('/publish-status-answer-02-details', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/02-details', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/02-details?reference=${foundCase.reference}`);
});
