...
router.get('/current-service/back-office/tab-view/13-residential', function(req, res) {
  ...
  // Render the page
  res.render('current-service/back-office/tab-view/13-residential', { 
...
router.post('/publish-status-answer-13-residential', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/13-residential', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/13-residential?reference=${foundCase.reference}`);
});
