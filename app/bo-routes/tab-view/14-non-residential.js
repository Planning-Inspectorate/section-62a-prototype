...
router.get('/current-service/back-office/tab-view/14-non-residential', function(req, res) {
  ...
  // Render the page
  res.render('current-service/back-office/tab-view/14-non-residential', { 
...
router.post('/publish-status-answer-14-non-residential', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/14-non-residential', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/14-non-residential?reference=${foundCase.reference}`);
});
