...
router.get('/current-service/back-office/tab-view/05-dates', function(req, res) {
...
router.post('/publish-status-answer-05-dates', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/05-dates', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/05-dates?reference=${foundCase.reference}`);
});
