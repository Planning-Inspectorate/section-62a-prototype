...
router.get('/current-service/back-office/tab-view/16-waste', function(req, res) {
  ...
  // Render the page
  res.render('current-service/back-office/tab-view/16-waste', { 
...
router.post('/publish-status-answer-16-waste', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/16-waste', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/16-waste?reference=${foundCase.reference}`);
});
