...
router.get('/current-service/back-office/tab-view/07-case-team', function(req, res) {
  ...
  // Render the page
  res.render('current-service/back-office/tab-view/07-case-team', { 
...
router.post('/publish-status-answer-07-case-team', function (req, res) {
  const foundCase = getCase(req);
  ...
  if (foundCase.publishStatus !== "Yes") {
    ...
    if (!hasAddress && !hasCoords) {
      return res.render('current-service/back-office/tab-view/07-case-team', {
        currentCase: foundCase,
  ...
  res.redirect(`/current-service/back-office/tab-view/07-case-team?reference=${foundCase.reference}`);
});
